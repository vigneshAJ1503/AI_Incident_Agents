...
│   └── traffic-generator/
│
├── sample-repo/                       # Seed script that builds a local git repo with history
│   └── build_history.sh               #   (incl. the S1 "bad commit")
│
├── knowledge-base/
...
- **Scope:** `sample-repo/build_history.sh` creates about 30 realistic commits, including a deliberately faulty `tune db pool` commit (`DB_POOL_SIZE 20→2`) tagged `v1.8.2`, matching the deployed version.
